...
async function runEntityDetailPageTests(page, screenshots, results, runTest, submitActions = false) {
    ...
    await runTest('Customer Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            const detailCard = page.locator('.detail-card, .card, .detail-grid');
    ...
    await runTest('Customer Detail - Direct URL', async () => {
        ...
        const heading = page.locator('h1, h2, .page-title, .customer-name');
        ...
        const toggleBtn = page.locator('button:has-text("Activate"), button:has-text("Deactivate")');
    ...
    await runTest('Material Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            const typeBadge = page.locator('[class*="type-"], .material-type-badge');
    ...
    await runTest('Material Detail - Direct URL', async () => {
        ...
        const codeField = page.locator('text=Material Code, text=materialCode');
        if (await codeField.isVisible({ timeout: 2000 })) {
            console.log('   Material code field visible');
        }

        // Check for base unit
        const unitField = page.locator('text=Base Unit, text=baseUnit');
    ...
    await runTest('Product Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            const skuField = page.locator('text=SKU, .sku-display');
    ...
    await runTest('Product Detail - View BOM Link', async () => {
        ...
        const bomBtn = page.locator('button:has-text("BOM"), button:has-text("Bill of Materials"), a:has-text("BOM")');
    ...
    await runTest('Equipment Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            const capacityField = page.locator('text=Capacity, text=capacity');
    ...
    await runTest('Equipment Detail - Action Buttons', async () => {
        ...
        const maintenanceBtn = page.locator('button:has-text("Maintenance"), button:has-text("Start Maintenance")');
        const holdBtn = page.locator('button:has-text("Hold"), button:has-text("Put on Hold")');
        const releaseBtn = page.locator('button:has-text("Release"), button:has-text("End Maintenance")');
        const editBtn = page.locator('button:has-text("Edit")');
    ...
    await runTest('Operator Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            const codeField = page.locator('text=Operator Code, text=operatorCode');
            if (await codeField.isVisible({ timeout: 2000 })) {
    ...
    await runTest('Operator Detail - Direct URL and Actions', async () => {
        ...
        const departmentField = page.locator('text=Department, text=department');
        const shiftField = page.locator('text=Shift, text=shift');
    ...
    await runTest('User Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            // Check for user info
            const emailField = page.locator('text=Email, text=email');
    ...
    await runTest('User Detail - Direct URL', async () => {
        ...
        const lastLogin = page.locator('text=Last Login, text=lastLogin, text=Never');
    ...
    await runTest('Hold Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            // Check for hold info
            const entityTypeBadge = page.locator('[class*="entity-"], .entity-type-badge');
    ...
    await runTest('Hold Detail - Direct URL', async () => {
        ...
        const reasonField = page.locator('text=Reason, .hold-reason');
        if (await reasonField.isVisible({ timeout: 2000 })) {
            console.log('   Reason section visible');
        }

        // Check for applied/released info
        const appliedBy = page.locator('text=Applied By, text=Applied On');
    ...
    await runTest('Batch Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            const batchNum = page.locator('.batch-number, text=Batch Number');
    ...
    await runTest('Batch Detail - Full Content Check', async () => {
        ...
        const sections = {
            'Batch Number': page.locator('text=Batch Number'),
            'Material': page.locator('text=Material'),
            'Quantity': page.locator('text=Quantity'),
    ...
    await runTest('Batch Detail - Genealogy Section', async () => {
        ...
        const genealogySection = page.locator('text=Genealogy, text=Traceability, text=Source Materials, text=Parent');
        if (await genealogySection.isVisible({ timeout: 3000 })) {
            console.log('   Genealogy section visible');
        }

        // Check for parent/child batch links
        const parentBatches = page.locator('.parent-batch, .source-material, text=Source');
        const childBatches = page.locator('.child-batch, .derived-product, text=Derived');
        ...
        const chart = page.locator('canvas, .echarts, [class*="chart"], [class*="genealogy-chart"]');
    ...
    await runTest('Batch Detail - Quality Approval Status', async () => {
        ...
        const qualitySection = page.locator('text=Quality, text=Approval, text=Approved, text=Pending');
    ...
    await runTest('Batch Detail - Allocations Section', async () => {
        ...
        const allocSection = page.locator('text=Allocation, text=Order Allocation');
        if (await allocSection.isVisible({ timeout: 2000 })) {
            console.log('   Allocations section visible');

            // Check for availability summary
            const availSummary = page.locator('text=Available, text=Allocated, text=Total');
    ...
    await runTest('Inventory Detail - Navigate from List', async () => {
        ...
        if (await row.isVisible({ timeout: 3000 })) {
            ...
            const typeBadge = page.locator('[class*="type-"], .inventory-type-badge');
            if (await typeBadge.isVisible({ timeout: 2000 })) {
                const type = await typeBadge.textContent();
    ...
    await runTest('Inventory Detail - Content and Actions', async () => {
        ...
        const materialName = page.locator('text=Material, .material-name');
        const quantity = page.locator('text=Quantity, .quantity-display');
        const location = page.locator('text=Location, .location');
    ...
    await runTest('Inventory Detail - Batch Link', async () => {
        ...
        const batchSection = page.locator('text=Batch, .batch-info');
        if (await batchSection.isVisible({ timeout: 2000 })) {
            console.log('   Batch information section visible');

            // Check for clickable batch link
            const batchLink = page.locator('a[href*="batches"], .batch-link, a:has-text("Batch")');
    ...
    await runTest('Order Detail - Content Sections', async () => {
        ...
        const orderNumber = page.locator('text=Order Number, text=Order #, .order-number');
        const customer = page.locator('text=Customer');
    ...
    await runTest('Order Detail - Line Items', async () => {
        ...
        const lineItemsSection = page.locator('text=Line Item, text=Products, h3:has-text("Line")');
    ...
    await runTest('Order Detail - Operations Timeline', async () => {
        ...
        const opsSection = page.locator('text=Operations, text=Process Flow, .process-flow, .operations-timeline');
