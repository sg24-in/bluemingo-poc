...
import { HoldReason, PagedResponse, PageRequest, DEFAULT_PAGE_SIZE } from '../../../shared/models';

@Component({
  selector: 'app-hold-reasons-list',
  templateUrl: './hold-reasons-list.component.html',
  styleUrls: ['./hold-reasons-list.component.css']
})
export class HoldReasonsListComponent implements OnInit {
  items: HoldReason[] = [];
  loading = true;
  page = 0;
  ...
  itemToDelete: HoldReason | null = null;
  deleteLoading = false;
  deleteError = '';
  ...
  loadItems(): void {
    ...
    this.apiService.getHoldReasonsPaged(request).subscribe({
      next: (response: PagedResponse<HoldReason>) => {
  ...
  create(): void { this.router.navigate(['/manage/config/hold-reasons/new']); }
  edit(item: HoldReason): void { this.router.navigate(['/manage/config/hold-reasons', item.reasonId, 'edit']); }
  openDeleteModal(item: HoldReason): void { this.itemToDelete = item; this.deleteError = ''; this.showDeleteModal = true; }
  closeDeleteModal(): void { this.showDeleteModal = false; this.itemToDelete = null; }
  confirmDelete(): void {
    if (!this.itemToDelete) return;
    this.deleteLoading = true;
    this.apiService.deleteHoldReason(this.itemToDelete.reasonId).subscribe({
