...
-- 6. Processes Table
-- Processes - Design-time entity per MES Consolidated Specification
...
-- 7b. Operation Templates Table (DESIGN-TIME)
CREATE TABLE IF NOT EXISTS operation_templates (
    operation_template_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
-- 9. Operations Table
-- Operations - Links to Process (design-time) and OrderLineItem (runtime) per MES Consolidated Spec
...
-- 35. Batch Quantity Adjustments Table
...
-- 36. Batch Size Configuration Table
...
-- idx_processes_order_line removed: order_line_id no longer exists in processes table (MES Consolidated Spec)
